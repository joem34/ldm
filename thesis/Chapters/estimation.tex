...
The first variable $intrametro_{ij}$ identifies trips within the same zone, where that zone is a metropolitan zone. This allows the model to reflect the propensity of a traveler to leave a metropolitan zone when they travel. The second, $intermetro_{ij}$ is $1$ when the traveler is traveling from one metropolitan zone to another and $0$ otherwise. This may be a common choice for business travelers, but less likely for recreational trips. The third variable, $intrarural_{ij}$ allows the model to consider the intra-zonal behavior in larger, rural zones. The inclusion of these variables significantly improves the model results, and as presented in~\ref{table:m3-coeff}, particularly for business travel.
